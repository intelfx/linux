...
static struct device_node *of_find_matching_node_and_match(struct device_node *from,
...
static void clk_summary_show_one(struct seq_file *s, struct clk *c, int level)
{
	...
	seq_printf(s, "%*s%-*s %-11d %-12d %-10lu %-11lu",
		   level * 3 + 1, "",
		   30 - level * 3, c->name,
		   c->enable_count, c->prepare_count, clk_get_rate_dk(c),
		   clk_get_accuracy(c));
	seq_printf(s, "\n");
}
...
static void clk_dump_one(struct seq_file *s, struct clk *c, int level)
{
	...
	seq_printf(s, "\"rate\": %lu", clk_get_rate_dk(c));
...
EXPORT_SYMBOL(clk_unprepare_dk);
