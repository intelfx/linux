...
static bool is_mach_ipq(void) {
	if (rb_mach == RB_MACH_IPQ806X) return true;
	if (rb_mach == RB_MACH_IPQ40XX) return true;
	return false;
}
...
#include <asm/io.h>
#define WDT_RST		(MSM_TMR0_BASE + 0x38 + 0x00)
#define WDT_EN		(MSM_TMR0_BASE + 0x38 + 0x08)
#define WDT_BARK	(MSM_TMR0_BASE + 0x38 + 0x14)
#define WDT_BITE	(MSM_TMR0_BASE + 0x38 + 0x24)
static int ipq_wdt_keepalive(void) {
    __raw_writel(327680, WDT_BITE);
    __raw_writel(327680, WDT_BARK);
    __raw_writel(1, WDT_RST);
    return 0;
}
static void ipq_wdt_enable(void) {
    ipq_wdt_keepalive();
    __raw_writel(1, WDT_EN);
}
static void ipq_wdt_disable(void) {
    ipq_wdt_keepalive();
