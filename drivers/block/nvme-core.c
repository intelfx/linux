...
static u64 nvme_readq(volatile void *ptr) {
#ifndef CONFIG_64BIT
    return ((u64) readl(ptr + 4) << 32) | readl(ptr);
#else
    return readq(ptr);
#endif    
}
...
nvme_alloc_iod(unsigned nseg, unsigned nbytes, struct nvme_dev *dev,
	       gfp_t gfp)
{
...
#ifndef CONFIG_64BIT
	writel(nvmeq->sq_dma_addr, &dev->bar->asq);
	writel(nvmeq->cq_dma_addr, &dev->bar->acq);
#else	
...
static int nvme_setup_io_queues(struct nvme_dev *dev)
{
	...
	q_depth = min_t(int, NVME_CAP_MQES(nvme_readq(&dev->bar->cap)) + 1,
								NVME_Q_DEPTH);
	for (i = 0; i < nr_io_queues; i++) {
