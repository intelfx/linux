		...
		void __iomem		*iobase_app)
{
	...
	device->crypto_dma_params.regs_base = device->udma_regs_base;

	device->crypto_dma_params.name =
		kmalloc(strlen(dev_name(&device->pdev->dev)) + 1, GFP_KERNEL);
	if (device->crypto_dma_params.name == NULL) {
		dev_err(&device->pdev->dev, "kmalloc failed\n");
		err = -ENOMEM;
		goto done;
	}

	strcpy(device->crypto_dma_params.name, dev_name(&device->pdev->dev));

	device->crypto_dma_params.num_of_queues = max_channels;

	err = al_crypto_dma_init(&device->hal_crypto,
			&device->crypto_dma_params);
	if (err) {
	...
	rc = al_crypto_dma_state_set(&device->hal_crypto, UDMA_NORMAL);
...
err_no_chan:
	kfree(device->crypto_dma_params.name);
done:
	return err;
}
...
static int al_crypto_setup_interrupts(struct al_crypto_device *device)
{
	...
	for (i = 0; i < device->num_channels; i++) {
		...
		devm_alloc_num++;
                ...
                // setting affinity somehow is ignored
                cpu = i;
                cpumask_set_cpu(cpu, &chan->affinity_hint_mask);
		err = irq_set_affinity_hint(msix->vector, &chan->affinity_hint_mask);
		if (err) {
                    printk(dev, "irq_set_affinity_hint failed! %d\n", err);
		}

		err = irq_set_affinity(msix->vector, cpumask_of(cpu));
                if (err) {
                    printk("irq_set_affinity: %d\n", err);
                }
	}
...
static int al_crypto_alloc_chan_resources(struct al_crypto_chan *chan)
{
	...
	rc = al_crypto_dma_q_init(&device->hal_crypto, chan->idx,
...
/******************************************************************************
 *****************************************************************************/
/* Dma map of scatterlist, with chaining support
 */
...
/******************************************************************************
 *****************************************************************************/
/* Dma unmap of scatterlist, with chaining support
 */
