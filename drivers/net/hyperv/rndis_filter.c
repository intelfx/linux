			     ...
			     struct hv_netvsc_packet *pkt)
{
	...
	if (isvlan)
		rndisMessageSize += NDIS_VLAN_PPI_SIZE;
	...
	if (isvlan)
		rndisPacket->data_offset += NDIS_VLAN_PPI_SIZE;
	rndisPacket->data_len = pkt->total_data_buflen;

	if (pkt->vlan_tci & VLAN_TAG_PRESENT) {
		struct ndis_pkt_8021q_info *vlan;
		struct rndis_per_packet_info *ppi;

		rndisPacket->per_pkt_info_offset = sizeof(struct rndis_packet);
		rndisPacket->per_pkt_info_len = NDIS_VLAN_PPI_SIZE;

		ppi = (struct rndis_per_packet_info *)
		    ((ulong) rndisPacket + rndisPacket->per_pkt_info_offset);
		ppi->size = NDIS_VLAN_PPI_SIZE;
		ppi->type = IEEE_8021Q_INFO;
		ppi->ppi_offset = sizeof(struct rndis_per_packet_info);

		vlan = (struct ndis_pkt_8021q_info *)
