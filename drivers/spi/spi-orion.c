...
#define ORION_SPI_SPR_POS		0
#define ORION_SPI_SPR_MASK		(0x07UL << ORION_SPI_SPR_POS)
#define ORION_SPI_SPPR0_POS		4
#define ORION_SPI_SPPR0_MASK		(0x01UL << ORION_SPI_SPPR0_POS)
#define ORION_SPI_SPPR_HI_POS		6
#define ORION_SPI_SPPR_HI_MASK		(0x03UL << ORION_SPI_SPPR_HI_POS)
#define ORION_SPI_CLK_PRESCALE_MASK	\
	(ORION_SPI_SPR_MASK | ORION_SPI_SPPR0_MASK | ORION_SPI_SPPR_HI_MASK)
#define ORION_SPI_CLK_MAX_DIVIDER	(7L * (1L << 7L))
...
static int orion_spi_baudrate_set(struct spi_device *spi, unsigned int speed)
{
	...
	if (rate > ORION_SPI_CLK_MAX_DIVIDER)
		return -EINVAL;
	...
	/* spi_clk = sys_clk / (mul * (2 ^ pwr)) */
	for (pwr = 0; pwr <= 7; ++pwr) {
		mul = (rate / (1 << pwr)); // (2 ^ pwr)
		if (mul <= 7) {
			break;
		}
	}
	...
	prescale = (mul <<  ORION_SPI_SPR_POS)
		| ((pwr & 0x01) << ORION_SPI_SPPR0_POS)
		| (((pwr & 0x06) >> 1) << ORION_SPI_SPPR_HI_POS);
...
static struct orion_spi_dev orion_spi_dev_data = {
	.typ = ORION_SPI,
	.min_divisor = 1,
	.max_divisor = ORION_SPI_CLK_MAX_DIVIDER,
