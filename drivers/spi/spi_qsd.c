...
static struct clk *msm_clk_get(struct device *dev, const char *name)
{
	if (rb_mach != RB_MACH_IPQ40XX)
		return clk_get(dev, name);
	else
		return of_clk_get_by_name( dev->of_node, name);
}

static unsigned long msm_clk_round_rate(struct clk *clk, unsigned long rate)
{
	if (rb_mach != RB_MACH_IPQ40XX)
		return clk_round_rate(clk, rate);
	else
		return clk_round_rate_dk(clk, rate);
}

static int msm_clk_set_rate(struct clk *clk, unsigned long rate)
{
	if (rb_mach != RB_MACH_IPQ40XX)
		return clk_set_rate(clk, rate);
	else
		return clk_set_rate_dk(clk, rate);
}

static int msm_clk_prepare_enable(struct clk *clk)
{
	if (rb_mach != RB_MACH_IPQ40XX)
		return clk_prepare_enable(clk);
	else
		return clk_prepare_enable_dk(clk);
}

static void msm_clk_disable_unprepare(struct clk *clk)
{
	if (rb_mach != RB_MACH_IPQ40XX)
		return clk_disable_unprepare(clk);
	else
		return clk_disable_unprepare_dk(clk);
}
...
static inline void msm_spi_free_gpios(struct msm_spi *dd)
{
	...
	if (!dd->pdata->use_pinctrl) {
		...
		int result = pinctrl_select_state(dd->pinctrl, dd->pins_sleep);
		if (result)
			dev_err(dd->dev, "%s: Can not set %s pins\n",
...
static long msm_spi_clk_max_rate(struct clk *clk, unsigned long rate)
{
	...
	cur =  msm_clk_round_rate(clk, rate);
	if (cur == rate)
		return rate;

	/* if we got here then: cur > rate */
	lowest_available =  msm_clk_round_rate(clk, 0);
	...
	while (max_steps-- && step_size) {
		...
		cur =  msm_clk_round_rate(clk, guess);
...
static void msm_spi_clock_set(struct msm_spi *dd, int speed)
{
	...
	rc = msm_clk_set_rate(dd->clk, rate);
	if (!rc)
		dd->clock_speed = rate;
}
...
msm_spi_use_dma(struct msm_spi *dd, struct spi_transfer *tr, u8 bpw)
{
	...
	if (is_vmalloc_addr((const void *)rx) &&
		(rx & PAGE_MASK) != ((rx + tr->len) & PAGE_MASK))
		return false;
	else if (is_vmalloc_addr((const void *)tx) &&
	...
	*/
	...
	if (dd->mode != SPI_BAM_MODE) {
		do {
			...
			if ((op & SPI_OP_IP_FIFO_NOT_EMPTY) &&
			    dd->rx_bytes_remaining > 0) {
				msm_spi_read_word_from_fifo(dd);
			}
			if (!(op & SPI_OP_OUTPUT_FIFO_FULL) &&
			    dd->tx_bytes_remaining > 0) {
				msm_spi_write_word_to_fifo(dd);
			}
...
static void put_local_resources(struct msm_spi *dd)
{
	...
	msm_clk_disable_unprepare(dd->clk);
	msm_clk_disable_unprepare(dd->pclk);
...
static int get_local_resources(struct msm_spi *dd)
{
	...
	ret = msm_clk_prepare_enable(dd->clk);
	if (ret)
		goto clk0_err;
	ret = msm_clk_prepare_enable(dd->pclk);
	if (ret)
		goto clk1_err;
#if USE_IRQ
...
{
	int res = 0;
	res = msm_spi_prepare_transfer_hardware(spi->master);
	if (res) {
		msm_spi_unprepare_transfer_hardware(spi->master);
		return res;
	}
	res = msm_spi_transfer_one_message(spi->master, msg);
	msm_spi_unprepare_transfer_hardware(spi->master);
	return res;
}
...
skip_dma_resources:
	...
	dd->dev = &pdev->dev;
	dd->clk = msm_clk_get(&pdev->dev, "core_clk");
	if (IS_ERR(dd->clk)) {
	...
	dd->pclk = msm_clk_get(&pdev->dev, "iface_clk");
	if (IS_ERR(dd->pclk)) {
	...
	rc = msm_clk_prepare_enable(dd->clk);
	if (rc) {
	...
	clk_enabled = 1;
	rc = msm_clk_prepare_enable(dd->pclk);
	if (rc) {
