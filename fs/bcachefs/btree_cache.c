...
{
	struct btree_cache_list *list = container_of(shrink, struct btree_cache_list,
						     shrink);
	struct btree_cache *bc = container_of(list, struct btree_cache, live[list->idx]);
...
void bch2_fs_btree_cache_exit(struct bch_fs *c)
{
	...
	unregister_shrinker(&bc->live[1].shrink);
	unregister_shrinker(&bc->live[0].shrink);
...
int bch2_fs_btree_cache_init(struct bch_fs *c)
{
	...
	shrink = &bc->live[0].shrink;
	shrink->count_objects	= bch2_btree_cache_count;
	shrink->scan_objects	= bch2_btree_cache_scan;
	shrink->seeks		= 2;
	ret = register_shrinker(shrink, "%s-btree_cache", c->name);
	if (ret)
		goto err;

	shrink = &bc->live[1].shrink;
	shrink->count_objects	= bch2_btree_cache_count;
	shrink->scan_objects	= bch2_btree_cache_scan;
	shrink->seeks		= 8;
	ret = register_shrinker(shrink, "%s-btree_cache-pinned", c->name);
	if (ret)
		goto err;
